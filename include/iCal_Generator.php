...
class iCal_Generator
{
     ...
     * @return bool
     */
    public static function iCalGener($timetable, $remove, $cal_name, $cal_title, $for_group, $rewrite = false)
    {
        echo 'http://' . $_SERVER['SERVER_NAME'] . '/' . self::ICAL_DIRECTORY . '/' . $cal_name . self::DOT_ICS;
        $file_exists = file_exists(self::ICAL_DIRECTORY_PATH . self::ICAL_DIRECTORY . '/' . $cal_name . self::DOT_ICS);
        if ($rewrite || !$file_exists) {
